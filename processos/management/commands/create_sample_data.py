...
def _get_or_create_lookup(model, field_name, values):
    """Cria registros de catálogo ausentes e retorna queryset filtrado."""
    for value in values:
...
def _random_cpf():
    """Gera CPF válido utilizando Faker."""
    return fake.cpf()


def _random_cnpj():
    """Gera CNPJ válido utilizando Faker."""
    return fake.cnpj()
...
class Command(BaseCommand):
    help = (
        "Gera processos de exemplo com dados fictícios realistas via Faker e "
        "cria catálogos necessários quando ainda não existirem."
    )

    def add_arguments(self, parser):
        """Define parâmetros opcionais de quantidade para geração de dados."""
        parser.add_argument(
            "--count",
            type=int,
            default=50,
            help="Quantidade de processos de exemplo a criar (padrão: 50).",
        )
        parser.add_argument(
            "--credores",
            type=int,
            default=20,
            help="Quantidade de credores de exemplo a criar (padrão: 20).",
        )

    def handle(self, *args, **options):
        """Executa a carga de dados fictícios de apoio ao desenvolvimento."""
        count = options["count"]
        ...
        self.stdout.write(
            "   Execute python manage.py runserver e acesse o sistema para visualizar os dados fictícios."
        )
