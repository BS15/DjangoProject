"""Views de documentos fiscais (notas) no fluxo de contas a pagar.
...
def api_toggle_documento_fiscal(request, processo_pk, documento_pk):
    ...
    Fluxo:
    - Se o documento já possuir nota fiscal vinculada, remove a nota e suas
...
def api_salvar_nota_fiscal(request, processo_pk, nota_pk):
    ...
    Entrada esperada (JSON ou form-data):
    - Dados da nota: emitente, número, data, valor bruto, fiscal de contrato,
      série e código de serviço INSS.
    - Flag de ateste da nota.
    - Listas paralelas de retenções: códigos, rendimentos, valores e
      beneficiários.

    Regras aplicadas:
    1. Atualiza os campos da nota com tolerância a dados opcionais/inválidos.
    2. Remove retenções anteriores e recria a partir do payload.
    3. Recalcula valor_liquido = valor_bruto - soma(retenções).
