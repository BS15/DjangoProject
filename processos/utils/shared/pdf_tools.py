"""Utilitários para manipulação, extração e mesclagem de arquivos PDF.

Consolidação de todas as operações de processamento de PDFs: leitura, extração de texto,
manipulação de páginas, e mesclagem. Não acessa banco de dados.

Módulo canônico: use este ao invés de processos.utils.pdf_extractors ou 
        ...
        for p_idx in pages_dict["pagamento"]:
            ...
            if val_str:
                try:
                    ...
                    logger.warning(
                        "Falha ao converter valor líquido extraído do SISCAC: %s",
                        exc,
                    )
    ...
    for pagina_info in paginas_temp:
        ...
        for match_valor in padrao_valor.finditer(texto_flat):
            ...
            try:
                ...
                logger.debug("Valor inválido no comprovante '%s': %s", valor_str, exc)
...
# ============================================================================
# Mesclagem e Manipulação de PDFs
# ============================================================================
