...
class DocumentoFiscal(models.Model):
    ...
    nome_emitente = models.ForeignKey('cadastros.Credor', on_delete=models.PROTECT, blank=True, null=True)
...
class RetencaoImposto(models.Model):
    ...
    beneficiario = models.ForeignKey('cadastros.Credor', on_delete=models.PROTECT, blank=True, null=True, verbose_name="Beneficiário", related_name='retencoes')
...
def _upload_documento_pagamento_imposto(instance, filename):
    ...
    return _build_upload_path("retencoes", "pagamentos_impostos", competencia_str, _safe_filename(filename))
